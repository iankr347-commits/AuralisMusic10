...
import com.auralis.innertube.models.BrowseEndpoint
import com.auralis.innertube.models.GridRenderer
...
import com.auralis.innertube.models.MusicCarouselShelfRenderer
import com.auralis.innertube.models.MusicShelfRenderer
import com.auralis.innertube.models.PlaylistItem
import com.auralis.innertube.models.SearchSuggestions
import com.auralis.innertube.models.Run
import com.auralis.innertube.models.Runs
import com.auralis.innertube.models.SongItem
import com.auralis.innertube.models.WatchEndpoint
import com.auralis.innertube.models.WatchEndpoint.WatchEndpointMusicSupportedConfigs.WatchEndpointMusicConfig.Companion.MUSIC_VIDEO_TYPE_ATV
import com.auralis.innertube.models.YouTubeClient
import com.auralis.innertube.models.YouTubeClient.Companion.WEB
import com.auralis.innertube.models.YouTubeClient.Companion.WEB_REMIX
import com.auralis.innertube.models.YouTubeLocale
import com.auralis.innertube.models.getContinuation
import com.auralis.innertube.models.getItems
import com.auralis.innertube.models.oddElements
import com.auralis.innertube.models.response.AccountMenuResponse
import com.auralis.innertube.models.response.BrowseResponse
import com.auralis.innertube.models.response.CreatePlaylistResponse
import com.auralis.innertube.models.response.EditPlaylistResponse
import com.auralis.innertube.models.response.FeedbackResponse
import com.auralis.innertube.models.response.GetQueueResponse
import com.auralis.innertube.models.response.GetSearchSuggestionsResponse
import com.auralis.innertube.models.response.GetTranscriptResponse
import com.auralis.innertube.models.response.ImageUploadResponse
import com.auralis.innertube.models.response.NextResponse
import com.auralis.innertube.models.response.PlayerResponse
import com.auralis.innertube.models.response.SearchResponse
import com.auralis.innertube.pages.AlbumPage
import com.auralis.innertube.pages.ArtistItemsContinuationPage
import com.auralis.innertube.pages.ArtistItemsPage
import com.auralis.innertube.pages.ArtistPage
import com.auralis.innertube.pages.ChartsPage
...
import com.auralis.innertube.pages.HistoryPage
import com.auralis.innertube.pages.HomePage
import com.auralis.innertube.pages.LibraryContinuationPage
import com.auralis.innertube.pages.LibraryPage
import com.auralis.innertube.pages.MoodAndGenres
import com.auralis.innertube.pages.NewReleaseAlbumPage
import com.auralis.innertube.pages.NextPage
import com.auralis.innertube.pages.NextResult
import com.auralis.innertube.pages.PlaylistContinuationPage
import com.auralis.innertube.pages.PlaylistPage
import com.auralis.innertube.pages.RelatedPage
import com.auralis.innertube.pages.SearchPage
import com.auralis.innertube.pages.SearchResult
import com.auralis.innertube.pages.SearchSuggestionPage
import com.auralis.innertube.pages.SearchSummary
import com.auralis.innertube.pages.SearchSummaryPage
...
object YouTube {
    ...
    /**
     * Get geo-based music recommendations based on the user's country/region.
     * This fetches trending songs, top songs, new releases, and popular artists
     * for a specific geographic location.
     */
    suspend fun getGeoRecommendations(countryCode: String? = null): Result<GeoRecommendationsPage> = runCatching {
        // Use the provided country code or fall back to locale setting
        val targetCountry = countryCode ?: locale.gl
        val countryName = getCountryName(targetCountry)
        
        // Create a custom locale with the target country for geo-specific results
        val geoLocale = YouTubeLocale(gl = targetCountry, hl = locale.hl)
        
        // Fetch charts data which contains geo-specific trending content
        val chartsResponse = innerTube.browse(
            client = WEB_REMIX,
            browseId = "FEmusic_charts",
            params = "ggMGCgQIgAQ%3D",
            customLocale = geoLocale
        ).body<BrowseResponse>()

        val trending = mutableListOf<YTItem>()
        val topSongs = mutableListOf<YTItem>()
        val newReleases = mutableListOf<YTItem>()
        val popularArtists = mutableListOf<YTItem>()

        chartsResponse.contents?.singleColumnBrowseResultsRenderer?.tabs?.firstOrNull()
            ?.tabRenderer?.content?.sectionListRenderer?.contents?.forEach { content ->
                content.musicCarouselShelfRenderer?.let { renderer ->
                    val title = renderer.header?.musicCarouselShelfBasicHeaderRenderer?.title?.runs?.firstOrNull()?.text
                        ?: return@forEach
                    ...
                    when {
                        title.contains("Trending", ignoreCase = true) -> trending.addAll(items)
                        title.contains("Top", ignoreCase = true) && title.contains("song", ignoreCase = true) -> topSongs.addAll(items)
                        title.contains("artist", ignoreCase = true) -> popularArtists.addAll(items)
                        else -> {} // Skip other sections
                    }
                }
                ...
                content.gridRenderer?.let { renderer ->
                    ...
                    val items = renderer.items.mapNotNull { item ->
                        item.musicTwoRowItemRenderer?.let { twoRowRenderer ->
                            convertMusicTwoRowItem(twoRowRenderer)
                        }
                    }.filterNotNull()

                    if (title.contains("release", ignoreCase = true) || title.contains("new", ignoreCase = true)) {
                        newReleases.addAll(items)
                    }
                }
            }

        // Also try to get explore page for more new releases if needed
        if (newReleases.isEmpty()) {
            try {
                val exploreResponse = innerTube.browse(
                    client = WEB_REMIX,
                    browseId = "FEmusic_explore",
                    customLocale = geoLocale
                ).body<BrowseResponse>()
                exploreResponse.contents?.singleColumnBrowseResultsRenderer?.tabs?.firstOrNull()
                    ...
                    ?.mapNotNull { it.musicTwoRowItemRenderer }
                    ?.mapNotNull { convertMusicTwoRowItem(it) }
                    ?.let { newReleases.addAll(it) }
            } catch (e: Exception) {
                // Ignore errors fetching explore page, charts data is primary
            }
        }

        GeoRecommendationsPage(
            countryCode = targetCountry,
            countryName = countryName,
            trending = trending.take(20),
            topSongs = topSongs.take(20),
            newReleases = newReleases.take(15),
            popularArtists = popularArtists.take(10)
        )
    }
