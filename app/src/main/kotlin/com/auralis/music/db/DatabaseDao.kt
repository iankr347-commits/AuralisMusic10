...
import com.auralis.music.constants.AlbumSortType
import com.auralis.music.constants.ArtistSongSortType
import com.auralis.music.constants.ArtistSortType
import com.auralis.music.constants.PlaylistSortType
import com.auralis.music.constants.SongSortType
import com.auralis.music.db.entities.Album
import com.auralis.music.db.entities.AlbumArtistMap
import com.auralis.music.db.entities.AlbumEntity
import com.auralis.music.db.entities.PlayCountEntity
import com.auralis.music.db.entities.AlbumWithSongs
...
import com.auralis.music.db.entities.SetVideoIdEntity
...
import com.auralis.music.extensions.reversed
...
import com.auralis.music.ui.utils.resize
...
interface DatabaseDao {
        ...
        suspend fun insertLibraryHistory(item: com.auralis.music.db.entities.LibraryHistoryEntity)
    ...
    fun eventsPagedRaw(limit: Int, offset: Int): Flow<List<EventWithSong>>
    
    // Wrapper with error handling for old backup compatibility
    fun eventsPaged(limit: Int, offset: Int): Flow<List<EventWithSong>> = flow {
        try {
            eventsPagedRaw(limit, offset).collect { events ->
                // Filter out any events with null or invalid data
                val validEvents = events.filter { event ->
                    try {
                        event.event.timestamp != null && 
                        event.song.id.isNotBlank() &&
                        event.song.title.isNotBlank()
                    } catch (e: Exception) {
                        false
                    }
                }
            ...
            // If query fails completely, emit empty list instead of crashing
            timber.log.Timber.tag("DatabaseDao").e(e, "Error querying events, returning empty list")
            emit(emptyList())
        }
    }
