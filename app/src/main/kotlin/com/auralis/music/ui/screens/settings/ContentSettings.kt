...
import com.auralis.music.constants.*
...
fun ContentSettings(
    ...
    val (geoRecommendations, onGeoRecommendationsChange) = rememberPreference(key = GeoRecommendationsKey, defaultValue = true)
    val (geoSelectedCountry, onGeoSelectedCountryChange) = rememberPreference(key = GeoSelectedCountryKey, defaultValue = SYSTEM_DEFAULT)
    ...
    Column(
        ...
        Material3SettingsGroup(
            ...
            items = listOf(
                ...
                Material3SettingsItem(
                    icon = painterResource(R.drawable.location_on),
                    title = { Text(stringResource(R.string.geo_recommendations)) },
                    description = { 
                        val selectedCountry = geoSelectedCountry
                        val countryName = if (selectedCountry == SYSTEM_DEFAULT) {
                            stringResource(R.string.system_default)
                        } else {
                            CountryCodeToName[selectedCountry] ?: selectedCountry
                        }
                        Text(
                            if (geoRecommendations) countryName else stringResource(R.string.off)
                        )
                    },
                    onClick = { navController.navigate("settings/content/geo_recommendations") }
                )
            )
        )
